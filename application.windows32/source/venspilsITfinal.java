...
public class venspilsITfinal extends PApplet {
...
public void calcco2(int m, float v, float v0) {
  float STEP = 0.0083333f;
  float PX = 0.12f;
  v *= PX/STEP;
  v0 *= PX/STEP;
  ...
  float zd = 0.000000236f*v*v*M;
  float zr = (0.0000372f*v + 0.0000000309f*v*v)*M;
  float za = 0.0000129f*Cd*A*v*v*v;
  float ze = 0.000278f*a*M*v;
...
public void makecar(int v) {
  ...
  case 0:
    ...
    carmas[cars][1]=-50;   //x
    carmas[cars][2]=540;   //y
    carmas[cars][8]=0.9f+random(20)/100.0f;     //maxv
    carmas[cars][3]=carmas[cars][8];     //dx
    carmas[cars][4]=0;     //dy
    carmas[cars][5]=0;     //t/l type
    carmas[cars][6]=1;     //show?
    carmas[cars][7]=0.02f+random(50)/1000.0f;  //minv
    if (cars==0) {
  ...
  case 0://###########################################################################
    for (int i=0; i<cars3; i++) {
      ...
      case 1:  //left to up
        ...
        rotate(radians(90-0.5f*carmas3[i][0]));
        if (carmas3[i][5]==0) {
          fill(255, 10, 10);
          rect(100, 0, 25, 40, 5);
        } else if (carmas3[i][5]==1) {
        ...
        carmas3[i][10]=PApplet.parseInt(screenX(100, 0));
        carmas3[i][11]=PApplet.parseInt(screenY(100, 0));
        popMatrix();
        carmas3[i][0]++;
        }
      ...
      case 2:        //right to up           
          ...
          carmas3[i][10]=PApplet.parseInt(screenX(50, 0));
          carmas3[i][11]=PApplet.parseInt(screenY(50, 0));
          popMatrix();
          carmas3[i][0]++;
          }
          if (carmas3[i][0]==180) carmas3[i][9] = 3; //for driving straight up
      ...
      case 3:  //up
        ...
        if(carmas3[i][11]>0){
          calcco2(round(carmas3[i][5]),1.0f,1.0f);
        }
        break;
      case 4:  //       
        if(carmas3[i][2]<445){
        ...
        rotate(radians(0.5f*carmas3[i][0]));
        ...
        carmas3[i][10]=PApplet.parseInt(screenX(50, 0)); //px
        carmas3[i][11]=PApplet.parseInt(screenY(50, 0)); //py
        popMatrix();
        carmas3[i][0]++;
        }
      ...
      case 5:  //
        ...
        rotate(radians(180-0.5f*carmas3[i][0]));
        ...
        carmas3[i][10]=PApplet.parseInt(screenX(100, 0)); //px
        carmas3[i][11]=PApplet.parseInt(screenY(100, 0)); //py
        popMatrix();
        carmas3[i][0]++;
        }
...
//text("Sets to 0 when simulation starts", 725, 85, 250, 110);
    ...
    for (int i=0; i<cars; i++) {
      ...
      if (carmas[i][6]==1) {
        ...
        if (oldspeed<carmas[i][3]) {
          if (carmas[i][3]-oldspeed>0.02f) {
            carmas[i][3]=oldspeed+0.02f;
            if (carmas[i][3]>carmas[i][8]/1.5f) {
              carmas[i][3]=carmas[i][8]/1.5f;
            }
          }
        }
        ...
        if (carmas[i][1]<1100&&carmas[i][3]>0.002f) {
    ...
    for (int i=0; i<cars1; i++) {
      ...
      if (carmas1[i][6]==1) {
        ...
        if (oldspeed>carmas1[i][3]) {
          if (abs(carmas1[i][3]-oldspeed)>0.02f) {
            carmas1[i][3]=oldspeed-0.02f;
            if (abs(carmas1[i][3])>carmas1[i][8]/1.5f) {
              carmas1[i][3]=-1*(carmas1[i][8]/1.5f);
            }
          }
        }
        ...
        if (carmas1[i][1]>-100&&carmas1[i][3]<(-0.002f)) {
    ...
    for (int i=0; i<cars2; i++) {
      if (carmas2[i][6]==1) {
        ...
        if (oldspeed<carmas2[i][3]) {
          if (carmas2[i][3]-oldspeed>0.02f) {
            carmas2[i][3]=oldspeed+0.02f;
            if (carmas2[i][3]>carmas2[i][8]/1.5f) {
              carmas2[i][3]=carmas2[i][8]/1.5f;
            }
          }
        }
     ...
     println(cars2pk);*/
    if (griezas==0) {
      if (carmas[carsfirst][3]<0.1f&&carspk==1&&carmas[carsfirst][6]==1) {
        if (carmas1[cars1first][3]>-0.1f&&cars1pk==1&&carmas1[cars1first][6]==1) {
          if (carmas2[cars2first][4]<0.1f&&cars1pk==1&&carmas2[cars2first][6]==1) {
            griezas=3;
            krustF();
          }
        }
  ...
  if (start0 == 2) {
            ...
            textSize(38);
            text(round(co2*1000*1000)/1000.0f+" g", 508, 330);
            textSize(20);
            ...
            text("Average people per second: "+round(people/(timeyes/1000.0f)*100)/100.0f, 508, 410);}
...
public void controlEvent(ControlEvent theEvent) {
 ...
 if (theEvent.getController().getName() == "ok") {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "venspilsITfinal" };
    if (passedArgs != null) {
